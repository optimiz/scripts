...
# 2021-06-01 - FE - Revise select to include non-returned students who exit in week(s) between active term dates.
...
$exitedstudents=Invoke-SqlQuery -query "SELECT student_web_id FROM students WHERE enroll_status > 0 AND schoolid IN (SELECT DISTINCT schoolid FROM terms WHERE lastday >= sysdate) AND exitcode IS NOT NULL AND ((exitdate BETWEEN (SELECT MIN(firstday) FROM terms WHERE firstday < sysdate ) AND (SELECT MAX(lastday) FROM terms WHERE firstday < sysdate )) OR (exitdate BETWEEN (SELECT MAX(lastday) FROM terms WHERE firstday < sysdate ) AND sysdate) OR (exitdate IS NOT NULL AND entrydate >= (SELECT MIN(firstday) FROM terms WHERE lastday >= sysdate))) AND student_web_id IS NOT NULL AND psguid IS NOT NULL"
